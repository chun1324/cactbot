  ...
  config: [
    {
      ...
      comment: {
        ...
        cn: `對於所有雙人集合，該選項指定了選擇誰與誰集合的優先順序。
           如果你想讓近戰和坦克集合 (假設集合沒有同時點這兩個人), 選擇含有“近戰 (melee)”的選項。
           近戰 (melee) 集合指的是 近戰+坦克 和 治療+遠端。職能集合指的是 坦克 + 治療 和 DPS + DPS。
           搭檔集合指的是 支援 + DPS 和 支援 + DPS (任何組合都有可能，支援位是坦克和治療)。
           如果隊伍中有兩名遠端 DPS 或近戰 DPS, 無論此配置選項如何, 它都不會報“近戰 (melees)”。
           沒有考慮對非標準陣容隊伍 (非 2 支援 + 2DPS) 構成的支援。`,
